...
- **Conceptualmente módulo de [Ascendant Equipment](https://gitlab.com/stalking-dragons/minecraft/ascendant-equipment)** (fork de Apotheosis): en el mod original, `apothic_attributes` no declaraba una dependencia dura de `apotheosis` en `neoforge.mods.toml` pese a compartir clases del sistema de afijos (compiladas directamente en el propio jar en vez de consumidas como dependencia externa). Replicamos esa misma decisión: **no se declara `ascendant_equipment` como dependencia obligatoria en el toml** todavía. Cuando Ascendant Equipment publique su API de afijos/atributos (aún no existe — a fecha de este scaffold `ascendant_equipment` está en `0.0.0-beta.1`, solo el esqueleto sin portar), esa API se consumirá como dependencia real de Gradle (`lib_ext/`) y se decidirá entonces si pasa a ser dependencia dura en el toml.
...
- Antes de portar el sistema de afijos/atributos: revisar el estado de `ascendant_equipment` (puede que ya haya avanzado desde este scaffold) y decidir si la dependencia pasa a ser dura en el toml
