...
| **9** | ✅ **Completada**: overlay de atributos en inventario, tooltips, HUD, partícula de crítico. `AttributesLibClient` renombrado a `AscendantAttributesClientHandler` (colisión con el stub del scaffold). Varios fixes de API real 26.1.2→26.2 (`I18n.exists`, `Gui.getMobEffectSprite`, `ChatFormatting.getColor/getChar`) | `client` (6: AttributeModifierComponent, AttributesGui, AttributesLibClient, ButtonPlacement, ModifierSource(Type)) | Fases 1-4, 5-6, 8 según feature |
| **10** | ✅ **Completada**: 7 mixins verificados en runtime (`runServer` con los 6 de servidor aplicados, cero fallos). Infraestructura de mixins creada desde cero (`ascendant_attributes.mixins.json` + `[[mixins]]` en toml). Colisión de nombre `CombatRules` (vanilla vs. propio) resuelta cualificando el vanilla. Confirmado sin solapamiento real con el mixin homónimo de Common Toolkit (interfaz marcador vacía) | `mixin` (7, incl. `mixin/client/AbstractContainerScreenMixin`) | Todas las anteriores relevantes |
| **11** | ✅ **Completada**: integración de slots de accesorio de Curios, verificado el arranque tanto con Curios cargado como sin él | `compat` (2: CurioEquipmentSlot, CuriosCompat) | Fases 1-4, 5-6 |
| **12** | ✅ **Completada**: arte propio (usuario) para los 7 mob effects, GUI de atributos, botones y sonido de esquiva. `lang/en_us.json` completo (165 claves). Importante: los mob effect icons **no** llevan nombre propio (`bleeding.png`, etc.) — Minecraft deriva la ruta del ID del efecto automáticamente, renombrarlos rompería el render; el resto de assets sí usa nombres propios (`ascendant_*`) | — (todo `assets/`) | Trabajo paralelo, no bloquea el resto |
| **13** | ✅ Verificación automatizada completa (`clean build` + `runServer` — arranque limpio en 3.25s, todos los registros/mixins/datapack cargados sin errores propios del mod). El **QA de paridad jugado** (comparar comportamiento en partida real contra el original) lo hace el usuario directamente, mod ya subido a CurseForge | — | Todas |
